...
def _parse_repo_file(list_file: Path) -> list[dict]:
    ...
    for lineno, raw in enumerate(list_file.read_text(encoding="utf-8").splitlines(), 1):
        line = raw.strip()
...
def _acquire_repo(ref: str, workspace: Path, git_token: str | None,
    ...
    dest = workspace / safe
    if dest.exists():
        if any(p for p in dest.iterdir() if p.name != ".git"):
            print(f"  [batch] reusing existing checkout {dest}", file=sys.stderr)
            return dest
        print(f"  [batch] empty dir at {dest} — removing and re-cloning",
              file=sys.stderr)
        _rmtree_rw(dest)
        if dest.exists():
...
def _stage_repo(ref: str, app_dir: Path, slug: str, git_token: str | None) -> None:
    ...
    dest = app_dir / slug
    if dest.exists():
        print(f"  [batch] reusing existing local copy {dest}", file=sys.stderr)
        return
...
def _run_batch_grouped(entries: list[dict], workspace: Path,
    ...
    for n, (app_id, repos) in enumerate(sorted(groups.items()), 1):
        ...
        slugs = [_module_name_from(r["repo_name"]) for r in repos]
...
def _render_batch_summary(results: list[dict], list_file: Path, elapsed: float) -> str:
    ...
    for i, r in enumerate(results, 1):
        rep = f"`{r['report']}`" if r["report"] else ""
        # Scrub inline credentials, then neutralise table-breaking chars so a
        # crafted ref can't forge cells/rows in the rendered summary.
        src = (_scrub_url_secrets(r["ref"])
               .replace("\n", " ").replace("|", "\\|"))
        out.append(f"| {i} | {r['app_id']} | {r['module']} | {r['status']} | "
                   f"{r['findings']} | {r['elapsed']:.0f} | {rep} | {src} |")
    failed = [r for r in results if r["status"] != "OK"]
    if failed:
        out.extend(["", "## Failures", ""])
        for r in failed:
            out.extend([f"### {r['module']} (app {r['app_id']})",
                        f"- Source: `{_scrub_url_secrets(r['ref'])}`",
                        f"- Error: {_scrub_url_secrets(r['error'])}", ""])
    return "\n".join(out) + "\n"
