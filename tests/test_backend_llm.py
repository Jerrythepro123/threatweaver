...
def test_prompt_codex_dispatch(patched_backends):
    cfg = _Cfg(id="gpt-codex-example", via="codex")
    out = llm.prompt("hi", model=cfg)
    assert out == "PROMPT_RESULT"

    _, codex_calls = patched_backends["codex"]
    assert len(codex_calls) == 1
    assert codex_calls[0][2] == "gpt-codex-example"
...
def test_agentic_codex_dispatch_preserves_stream_callback(patched_backends):
    cfg = _Cfg(id="gpt-codex-example", via="codex")
    cb = lambda line: None
    out = llm.agentic("go", model=cfg, stream_cb=cb)
    assert out == "AGENTIC_RESULT"
    _, codex_calls = patched_backends["codex"]
    assert len(codex_calls) == 1
    assert codex_calls[0][3]["stream_cb"] is cb
